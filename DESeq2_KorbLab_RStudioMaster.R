packages = c('edgeR', 'DESeq2', 'tximport', 'readr', 'GenomicFeatures', 
             'tidyverse', 'ggrepel', 'fs', 'argparse', 'purrr', 'readxl', 'glue', 'magrittr')
...
# Set working directory to the project folder for this analysis
setwd('path/to/project/directory')

# Below are some variables needed for setting up the DESeq analysis. 
# Default values are already in place; alternatives are in blue notes
organism <- 'mouse' # or 'human'
analysis_name <- sprintf('Deseq2_analysis_%s', today) # or replace with your own descriptive name
significance_level = 0.05

# If your DESeq template is located within your project folder, then you can input a relative path
# here (i.e. './template_file.xlsx')
template_path = './deseq_template_file.xlsx'
template_path = 'Deseq_CLI_test.csv'
# Similarly for the path to the quant files (salmon 'quants' output folder),
# you can use a relative path if they are within the project folder or you 
# can specify the full path (i.e. copy the path to the quants folder and paste it below)
quants_path = './quants'

# Depending on which organism you're using, run one of the following lines
if (organism == 'mouse' ) {
...
# The following will create a deseq folder within your project if it isn't already there
folders = dir()
if (!(glue('{analysis_name}') %in% folders)) {
  dir.create(glue('deseq_{analysis_name}'))
}
paste0('Current working directory is: ', getwd()) # Just a check to make sure you are in the right place
...
# Read in the template csv. It will pop open so you can see it to make sure it looks correct
sampleInfo = read_csv(file = template_path, col_names = T) %>% 
  arrange(Comparison_level) %T>%
  View()
...
# This should then point to the quants folder your specified earlier
dir <- file.path(quants_path) 

#this is the directory with my Salmon outputs and I wirte their names into a vector 
samplenames <- list.files(dir) 
...
# This will make sure that the quant file names that you provided are actually present within the
# 'quants' folder
...
# from the list of files, I select the ones I want for the untreated and LPs comparison (from the list of file names I selected above), and put them in the correct order
samplenames_reorder <- sampleInfo$Quant_file_name 

# make your colData table here; this helps DESeq know how to set up the analysis
colData <- makeColData(sampleInfo, samplenames_reorder)
# this is a table that has the exact file names of the Salmon output, 
# and the other columns are descriptors of the experimental design that will be important for the DESeq2 analysis later on
print(colData)

# Now we can build a vector which points to our quantification files using this column of coldata. 
# We use names to name this vector with the run IDs as well.
files <- file.path(dir,colData$samplenames_reorder,"quant.sf")
names(files) <- colData$samplenames_reorder

# use Tximport to read in files correctly. dim gives dimension readout. 
# Should be the number of lines and the number of samples
txi <- tximport(files, type="salmon", tx2gene=tx2gene,  ignoreAfterBar = TRUE)

# Write out the count file for these samples; this is needed for any GEO upload in the future.
...
write_tsv(as_tibble(rownames(down)), 
          file = glue('{analysis_name}/{analysis_name}_down_ensembl.txt'),
          col_names = F)
write_tsv(as_tibble(rownames(up)), 
          file = glue('{analysis_name}/{analysis_name}_up_ensembl.txt'),
          col_names = F)
write_tsv(as_tibble(down$GeneSymbol), 
          file = glue('{analysis_name}/{analysis_name}_down_geneSymbol.txt'),
          col_names = F)
write_tsv(as_tibble(up$GeneSymbol), 
          file = glue('{analysis_name}/{analysis_name}_up_geneSymbol.txt'),
          col_names = F)
